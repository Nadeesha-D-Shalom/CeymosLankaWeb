...
function CoconutManager() {
  ...
  const loadProducts = async () => {
    const res = await fetch(`${API_BASE}/coconut_manager/get_coconut_products.php`);
  ...
  const addProduct = async () => {
    ...
    await fetch(`${API_BASE}/coconut_manager/add_coconut_product.php`, {
      method: "POST",
      body: fd
    });
  ...
  const openEdit = (p) => {
    ...
    if (p.image) {
      setPreviewImg(`${API_BASE.replace("/api", "")}/uploads/coconut_products/${p.image}`);
    }
  ...
  const updateProduct = async () => {
    ...
    await fetch(`${API_BASE}/coconut_manager/update_coconut_product.php`, {
      method: "POST",
      body: fd
    });
  ...
  const deleteProduct = async () => {
    await fetch(`${API_BASE}/coconut_manager/delete_coconut_product.php`, {
      method: "POST",
  ...
  return (
    <AdminLayout>
      <div className="pm-header">
        <h1 className="page-title">Coconut Product Manager</h1>
        <button className="pm-add-btn" onClick={() => setShowAdd(true)}>
          <i className="fas fa-plus"></i> Add Coconut Product
        </button>
      </div>
        ...
        {loading ? (
          ...
          products.map((p) => (
            <div className="pm-card" key={p.id}>
              <img
                src={`${API_BASE.replace("/api", "")}/uploads/coconut_products/${p.image}`}
                className="pm-img"
                alt={p.title}
              />

              <h3 className="pm-name">{p.title}</h3>
              <p className="pm-sub">{p.net_weight}</p>
              ...
              <div className="pm-actions">
                ...
                <button className="pm-delete" onClick={() => { setSelectedProduct(p); setShowDelete(true); }}>
      ...
      {showAdd && (
        <div className="pm-modal-overlay">
          <div className="pm-modal">
            <h2>Add Coconut Product</h2>

            <input name="title" className="pm-input" placeholder="Title" onChange={handleChange} />
            <textarea name="description" className="pm-textarea" placeholder="Description" onChange={handleChange} />
            <input name="net_weight" className="pm-input" placeholder="Net Weight" onChange={handleChange} />
